...
p1 <- ggplot(toplo, aes(x = date, y = chla)) +
  ...
  labs(
    ...
    title = 'Observed Chlorophyll-a and TN Load',
    subtitle = '1995 to 2024'
  ) +
  theme_minimal()
p2 <- ggplot(toplo, aes(x = date, y = tn_load_lag0 / 10)) +
  geom_line(color = '#F8766D') +
  labs(
    x = NULL,
    y = 'TN Load (0.1 x tons/month)',
    subtitle = '1995 to 2024'
  ) +
  theme_minimal()
...
pall <- ggplot(tomodind, aes(x = tn_load / 10, y = chla)) +
...
# create scatterplots of chla vs tn_load with linear fits for each lag
pall <- ggplot(tomodadd, aes(x = tn_load / 10, y = chla)) +
  geom_point(alpha = 0.5, size = 0.5) +
  geom_smooth(
    method = 'lm',
    formula = y ~ x,
...
padd <- pall + ppre + ppos + plot_layout(ncol = 1, axis_titles = 'collect')
padd
```
...
mod <- tomodadd |>
  ...
  mutate(
    model = map(data, function(x) {
      modall <- lm(log10(chla) ~ factor(mo) + log10(tn_load), data = x)
      modpre <- lm(
        log10(chla) ~ factor(mo) + log10(tn_load),
        data = x[x$yrcat == 'Before 2010', ]
      )
      modpos <- lm(
        log10(chla) ~ factor(mo) + log10(tn_load),
...
tomod1 <- tomodadd |>
  filter(lag %in% c('Lag 0', 'Lag 1', 'Lag 2', 'Lag 3')) |>
  pivot_wider(
    names_from = lag,
    values_from = tn_load
  )
...
toplo <- tomod1 |>
  mutate(
    pred_best = 10^predict(modbest)
  )
ggplot(toplo, aes(x = date, y = chla)) +
  geom_point() +
  geom_line(aes(y = pred_best), color = '#F8766D') +
  scale_y_log10() +
  theme_minimal() +
  theme(legend.position = 'bottom') +
  labs(
    x = NULL,
    y = 'Chlorophyll-a (µg/L)',
    title = 'Observed vs Predicted Chlorophyll-a',
    subtitle = 'Using Best Fit Model'
  )
```
