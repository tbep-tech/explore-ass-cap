...
The above models used to predict the assimilative capacity of Old Tampa Bay, particularly for the period after 2010, likely provide an overestimate of how much loading the bay segment can assimilate.  This is likely true because of the residual patterns - the model does not capture extreme values.  The model does not consider an inherently log-normal response variable and additional predictors that could improve the model fit to provide a more accurate estimate of how nitrogen load influences chlorophyll-a.
